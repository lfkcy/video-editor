...
export class PlaybackControlIntegrator {
  ...
  private syncPlaybackState(): void {
    ...
    if (currentState.playhead !== this.timelineStore.playhead) {
      this.timelineStore = currentState;
      const timeInSeconds = TimelineDataAdapter.timeUtils.msToSeconds(currentState.playhead);
 ...
 * 创建播放控制集成器实例
 */
export function createPlaybackControlIntegrator(options?: PlaybackControlOptions): PlaybackControlIntegrator {
