...
export class ClipOperationManager {
    ...
    try {
      ...
      try {
        for (const actionId of actionIds) {
          ...
          // 通过集成器处理分割
          const newActions = await this.timelineIntegrator.handleActionSplit(
            action,
            splitTime
          );
      ...
      console.log("分割片段成功:", {
        actionIds,
        splitTime,
        newActionsCount: allNewActions.length,
      });
      return result;
    } catch (error) {
    ...
    try {
      ...
      try {
        for (const actionId of actionIds) {
          ...
          // 注册映射关系
          this.videoClipService
            .getActionSpriteManager()
            .register(newAction, newSprite);
  ...
  private findActionById(actionId: string): TimelineAction | null {
    ...
    // 简化实现，实际可能需要更复杂的查找逻辑
    return this.videoClipService
      .getActionSpriteManager()
      .getSpriteByActionId(actionId)
  ...
  onOperation(
    callback: (type: ClipOperationType, result: ClipOperationResult) => void
  ): () => void {
