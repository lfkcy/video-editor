...
export class KeyboardShortcutManager {
  ...
  getCategories(): string[] {
    const categories = new Set(
      this.shortcuts.map((shortcut) => shortcut.category)
    );
  ...
  formatShortcut(shortcut: ShortcutDefinition): string {
    ...
    const modifierParts = shortcut.modifiers.map(
      (mod) => modifierMap[mod] || mod
    );
    const keyPart =
      keyMap[shortcut.key] ||
      shortcut.key.replace("Key", "").replace("Digit", "");
