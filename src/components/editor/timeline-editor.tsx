...
import {
  ...
  TimelineAction,
  TimelineDataAdapter,
} from "@/lib/timeline-data-adapter";
import {
  timelineStateAdapter,
  TimelineEditorData,
} from "@/lib/timeline-state-adapter";
import {
  timelineEventHandler,
  TimelineEventCallbacks,
} from "@/lib/timeline-event-handler";
import {
  createPlaybackControlIntegrator,
  PlaybackControlIntegrator,
} from "@/lib/playback-control-integrator";
import {
  createTimelinePerformanceOptimizer,
  ...
  (
    ...
    const handleKeyDown = useCallback(
      (e: KeyboardEvent) => {
        ...
        // 先尝试事件处理器的快捷键
        const handled = eventHandlerRef.current.handleKeyboardShortcut(
          e.code,
          modifiers
        );
        if (handled) {
    ...
    useEffect(() => {
      ...
      const eventCallbacks: TimelineEventCallbacks = {
        ...
        onActionResize: (action, row, newStart, newEnd) => {
          ...
          // 使用集成器处理调整大小
          if (timelineIntegratorRef.current) {
            return timelineIntegratorRef.current.handleActionResize(
              action,
              newStart,
              newEnd
            );
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-600 truncate">
                {TimelineDataAdapter.timeUtils.formatTime(
                  TimelineDataAdapter.timeUtils.secondsToMs(
      ...
      <div className={`timeline-editor-wrapper ${className}`}>
          ...
          <EnhancedTimelineToolbar
            ...
            onSplitAtPlayhead={() => {
              const currentTimeSeconds =
                TimelineDataAdapter.timeUtils.msToSeconds(playhead);
              eventHandlerRef.current.handleSplitAtPlayhead(currentTimeSeconds);
            }}
            onDeleteSelected={() =>
              eventHandlerRef.current.handleActionDelete(selectedClips)
            }
            onDuplicateSelected={() =>
              eventHandlerRef.current.handleDuplicateSelected()
            }
        ...
        <div className="timeline-status-bar h-8 bg-muted/30 border-t border-border flex items-center px-4 text-xs text-muted-foreground">
          <div className="flex items-center space-x-4">
            ...
            <span>
              总时长: {TimelineDataAdapter.timeUtils.formatTime(duration)}
            </span>
          </div>
        </div>
      </div>
