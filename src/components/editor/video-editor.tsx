...
import { MediaLibrary, TimelineEditorProvider, TimelineEditorContextType } from '../panels/media-library';
...
export function VideoEditor() {
  ...
  const { setDuration, selectedClips, playhead, isPlaying } = useTimelineStore();
    ...
    const initializeEditor = async () => {
      try {
        ...
        if (!videoPreviewRef.current) {
          console.warn('VideoPreview 容器未就绪');
          return;
        }

        // 1. 创建 VideoClipService
        ...
        // 3. 创建播放同步管理器
        if (!playbackSyncManagerRef.current && 
            videoClipServiceRef.current && 
        ...
        // 4. 创建片段操作管理器
        if (!clipOperationManagerRef.current && 
            videoClipServiceRef.current && 
            timelineIntegratorRef.current) {
        ...
        if (!keyboardShortcutManagerRef.current) {
          ...
          keyboardShortcutManagerRef.current = createKeyboardShortcutManager();
        ...
        if (isMounted) {
          setInitializationError(error instanceof Error ? error.message : '初始化失败');
          setIsLoading(false);
          // performanceMonitor.measureCustomMetric('initializationErrors', 1);
        }
      }
    };
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">请稍候，正在加载核心组件</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold mb-2 text-destructive">初始化失败</h2>
    ...
    <ErrorBoundary onError={(error, errorInfo) => {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 p-4 bg-muted/20">
            <div ref={videoPreviewRef} className="h-full">
              <VideoPreview />
            </div>
          </div>
      ...
      <footer className="h-8 px-4 bg-muted/30 border-t flex items-center justify-between text-xs text-muted-foreground">
        <div className="flex items-center space-x-4">
          ...
          <span>分辨率: {currentProject.settings.width}×{currentProject.settings.height}</span>
          <span>帧率: {currentProject.settings.fps}fps</span>
          {selectedClips.length > 0 && (
            <span className="text-primary">已选择: {selectedClips.length} 个片段</span>
          )}
        </div>
        <div className="flex items-center space-x-4">
