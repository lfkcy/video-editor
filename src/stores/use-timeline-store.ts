...
interface TimelineActions {
  ...
  // 拖拽操作
  startDrag: (operation: DragOperation, position: Point, clipIds: string[]) => void;
...
const zoomLevels: ZoomLevel[] = [
  { scale: 0.01, label: '1%', majorTickInterval: 60000, minorTickInterval: 10000 },
  { scale: 0.02, label: '2%', majorTickInterval: 30000, minorTickInterval: 5000 },
  { scale: 0.05, label: '5%', majorTickInterval: 10000, minorTickInterval: 2000 },
  { scale: 0.1, label: '10%', majorTickInterval: 5000, minorTickInterval: 1000 },
...
export const useTimelineStore = create<TimelineStore>()(
  devtools(
    (set, get) => ({
      ...
      zoomIn: () => {
        const { scale } = get();
        const currentIndex = zoomLevels.findIndex(level => level.scale >= scale);
      ...
      formatTime: (time, format = 'hms') => {
        ...
        switch (format) {
          ...
          case 'frames':
            ...
            return `${get().formatTime(time, 'hms')}.${frames.toString().padStart(2, '0')}`;
          case 'seconds':
