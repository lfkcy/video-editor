...
export function useDragAndDrop() {
  ...
  const handleDropOnTimeline = useCallback(
    async (
      ...
      try {
        ...
        // 计算放置时间
        const dropTime =
          timelinePosition.x / timelinePosition.scale +
          timelinePosition.scrollPosition;
        ...
        console.log(
          `Added clip ${clipId} to track ${trackId} at time ${dropTime}ms`
        );
      } catch (error) {
  ...
  // 处理文件直接拖拽到时间轴（从系统）
  const handleFileDropOnTimeline = useCallback(
    async (
      e: React.DragEvent,
      trackId: string,
      timelinePosition: { x: number; scale: number; scrollPosition: number }
      ...
      try {
        // 处理文件
        const processedFiles = await mediaProcessingService.processMediaFiles(
          files
        );
