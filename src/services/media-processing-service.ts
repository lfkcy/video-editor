import { MediaFile, MediaMetadata, MediaType, MediaProcessingTask, MediaProcessingResult } from '@/types';
...
export class MediaProcessingService {
  ...
  async processMediaFile(file: File): Promise<MediaFile> {
    ...
    try {
      ...
      if (mediaFile.type === 'video' || mediaFile.type === 'image') {
        mediaFile.thumbnailUrl = await this.generateThumbnail(file, mediaFile.type);
      }
      ...
      task.result = {
        ...
        keyframes: [],
        analysis: {
          hasAudio: mediaFile.metadata.channels ? mediaFile.metadata.channels > 0 : false,
  ...
  async processMediaFiles(files: File[]): Promise<MediaFile[]> {
    ...
    for (let i = 0; i < files.length; i++) {
      try {
        ...
        errors.push(`${files[i].name}: ${error instanceof Error ? error.message : '处理失败'}`);
      }
    }
   ...
   * 生成缩略图
   */
  private async generateThumbnail(file: File, type: MediaType): Promise<string> {
    if (type === 'image') {
