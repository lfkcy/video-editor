...
/**
 * 扩展的 TimelineAction 类型，添加 trackId 属性
 */
interface ExtendedTimelineAction extends TimelineAction {
  trackId?: string;
  name?: string;
}
...
export class VideoClipService {
  ...
  // 事件监听器
  private spriteAddedListeners: ((action: TimelineAction, sprite: VisibleSprite) => void)[] = [];
  ...
  private setupManagerIntegration(): void {
    ...
    this.actionSpriteManager.addListener((event, action, sprite) => {
      ...
      if (event === 'register') {
        this.spriteAddedListeners.forEach(listener => listener(action, sprite));
  ...
  async addVideoSpriteToTrack(
    ...
    try {
      ...
      const { action, sprite } = await this.avCanvasManager.addVideoClip(file);
  ...
  async addTextSpriteToTrack(
    ...
    try {
      const { action, sprite } = await this.avCanvasManager.addTextClip(text, style as any);
      
      (action as any).trackId = trackId;
      (action as any).name = `文字: ${text.substring(0, 10)}${text.length > 10 ? '...' : ''}`;
  ...
  async splitSprite(
    ...
    try {
      // 使用 AVCanvas 管理器分割精灵
      const newActions = await this.avCanvasManager.splitSprite(action, splitTime);
   ...
   * 事件监听器管理
   */
  onSpriteAdded(callback: (action: TimelineAction, sprite: VisibleSprite) => void): () => void {
...
export function isSupportedFormat(file: File): boolean {
  ...
  const supportedAudioFormats = ['audio/mp3', 'audio/wav', 'audio/ogg', 'audio/m4a'];
